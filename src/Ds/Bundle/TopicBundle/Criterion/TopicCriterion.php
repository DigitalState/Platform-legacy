...
namespace Ds\Bundle\TopicBundle\Criterion;
 ...
 */
class TopicCriterion implements Criterion
{
    /**
...
//        $subQuery
//            ->select([ 's.user' ])
//            ->from('\\Ds\\Bundle\\TopicBundle\\Entity\\Subscription', 's');

        $statement = $this->entityManager->getConnection()->prepare('
                      SELECT
                          `ns`.`user_id`
                      FROM
                          `ds_topic_subscription` `ns`
                      INNER JOIN
                          `ds_topic_subscription_channel` `nsc` ON `nsc`.`subscription_id` = `ns`.`id`' . ($channel ? ' AND `nsc`.`channel_id` = :channel' : '') . '
                      WHERE
                          `ns`.`topic_id` = :topic'
        );

        $parameters = [ 'topic' => $criterion->getOperand2() ];
